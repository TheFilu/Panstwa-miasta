...
function RoomContent({ 
  room, players, currentRound, myAnswers, categories, isHost, session, setLocation 
}: { 
  room: any, players: any[], currentRound: any, myAnswers: any[], categories: string[], isHost: boolean | undefined, session: any, setLocation: any 
}) {
  const { toast } = useToast();
  const startGame = useStartGame();
  const submitAnswers = useSubmitAnswers();
  ...
  // Sync local state when room data changes (from other players/server)
  useEffect(() => {
