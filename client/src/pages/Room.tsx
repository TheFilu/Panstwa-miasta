...
function RoomContent({
  ...
  useEffect(() => { setLocalRounds(room.totalRounds); }, [room.totalRounds]);
  useEffect(() => { setLocalTimer(room.timerDuration); }, [room.timerDuration]);
  ...
  const handleTimerChange = (val: number) => {
    ...
    updateSettings.mutate({ code: room.code, settings: { timerDuration: newDuration } });
  };
    ...
    return (
      <div className="min-h-screen flex flex-col p-4 md:p-8 max-w-4xl mx-auto">
        <header className="flex justify-between items-center mb-12">
          ...
          <div onClick={copyCode} className="cursor-pointer bg-white px-4 py-2 rounded-xl shadow-sm border border-border flex items-center gap-2 hover:bg-gray-50 transition">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-start">
          <GameCard title="Players" icon={<Users className="w-5 h-5" />} className="h-full">
            <div className="space-y-4">
              <AnimatePresence>
                {players.map((player) => (
                  <motion.div key={player.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex items-center gap-4 p-3 rounded-2xl bg-muted/50">
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat: string) => (
                    <div key={cat} className="bg-white border rounded-lg px-3 py-1 flex items-center gap-2">
                      ...
                      {isHost && <button onClick={() => handleRemoveCategory(cat)} className="text-red-500"><Trash2 className="w-4 h-4" /></button>}
                    </div>
                  ))}
                </div>
                {isHost && (
                  <div className="flex gap-2">
                    <Input placeholder="Add category..." value={newCategory} onChange={(e) => setNewCategory(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAddCategory()} />
                    <Button onClick={handleAddCategory} size="icon"><Plus className="w-5 h-5" /></Button>
                  </div>
                )}
              </div>
            </GameCard>
            ...
            <GameCard title="Settings" icon={<Settings className="w-5 h-5" />}>
               <div className="space-y-6">
                 <div className="space-y-2">
                   <label className="text-sm font-bold flex justify-between"><span>Rounds</span><span className="text-primary">{room.totalRounds}</span></label>
                   <input type="range" min="1" max="20" value={localRounds} onChange={(e) => isHost && handleRoundsChange(parseInt(e.target.value))} disabled={!isHost} className="w-full h-2 accent-primary" />
                 </div>
                 <div className="space-y-2">
                   <label className="text-sm font-bold flex justify-between"><span>Timer</span><span className="text-primary">{localTimer === null ? "OFF" : `${localTimer}s`}</span></label>
                   <input type="range" min="0" max="60" step="5" value={localTimer === null ? 0 : localTimer} onChange={(e) => isHost && handleTimerChange(parseInt(e.target.value))} disabled={!isHost} className="w-full h-2 accent-primary" />
                 </div>
               </div>
            </GameCard>

            {isHost ? (
              <Button size="lg" className="w-full h-16 text-xl shadow-xl" disabled={players.length < 2 || startGame.isPending} onClick={handleStart}>
                {startGame.isPending ? "Starting..." : "Start Game"}
              </Button>
            ) : (
              <div className="text-center p-6 bg-white rounded-3xl border">
                <Loader2 className="w-8 h-8 text-primary animate-spin mx-auto mb-3" />
                <p className="font-medium text-muted-foreground">Waiting for host...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <GameCard className="max-w-2xl w-full text-center py-12">
          ...
          <div className="space-y-4 max-w-md mx-auto">
            {sortedPlayers.map((player, index) => (
              <div key={player.id} className={`flex items-center gap-4 p-4 rounded-2xl border-2 ${index === 0 ? "border-yellow-400 bg-yellow-50" : "bg-muted/50"}`}>
    ...
    return (
      <div className="min-h-screen flex flex-col p-4 md:p-6 max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <span className="text-sm font-bold text-muted-foreground uppercase">Round {room.roundNumber}/{room.totalRounds}</span>
            <div className="flex items-center gap-3">
              <span className="text-4xl font-black">Letter</span>
              <div className="bg-primary text-white text-4xl font-black w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg">{currentRound.letter}</div>
            </div>
          </div>
          <div className="text-right">
            {timeLeft !== null && <div className="text-2xl font-black text-red-500 bg-red-50 px-4 py-2 rounded-xl flex items-center gap-2"><Clock /> {timeLeft}s</div>}
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t flex justify-center z-10">
            <Button size="lg" className="w-full max-w-md h-16 text-xl shadow-xl" disabled={hasSubmitted || submitAnswers.isPending}>
    ...
    <div className="min-h-screen flex flex-col p-4 md:p-8 max-w-4xl mx-auto">
              ...
              return (
                <div key={catId} className="flex items-center justify-between p-3 border rounded-xl">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-muted-foreground uppercase">{catId}</span>
                    <span className="font-medium text-lg">{ans?.word || "-"}</span>
                  </div>
                  {ans?.isValid === true && <div className="text-green-600 font-bold">+ {ans.points}</div>}
                  {ans?.isValid === false && <div className="text-red-500 font-bold">0</div>}
                </div>
              );
            })}
          </div>
      ...
      {isHost && isRoundOver && (
        <div className="fixed bottom-8 left-0 right-0 flex justify-center">
          <Button size="lg" className="shadow-xl animate-bounce" onClick={handleStart} disabled={startGame.isPending}>
