import { db } from "./db";
import {
  rooms, players, rounds, answers,
  type Room, type Player, type Round, type Answer,
  type CreateRoomRequest, type JoinRoomRequest,
  type InsertRoom, type InsertPlayer, type InsertAnswer
} from "@shared/schema";
...
export interface IStorage {
  ...
  // Answers
  submitAnswers(roundId: number, playerId: number, submissions: Record<string, string>): Promise<void>;
  getAnswers(roundId: number): Promise<Answer[]>;
  updateAnswerValidation(id: number, isValid: boolean, points: number, reason?: string): Promise<Answer>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async addUsedLetter(id: number, letter: string): Promise<Room> {
    ...
    const [updated] = await db.update(rooms).set({ usedLetters }).where(eq(rooms.id, id)).returning();
    return updated;
  }

  async addPlayer(roomId: number, name: string, isHost: boolean = false): Promise<Player> {
  ...
  async updatePlayerScore(id: number, points: number): Promise<Player> {
    ...
    const [updated] = await db.update(players).set({ score: player.score + points }).where(eq(players.id, id)).returning();
    return updated;
  }
  ...
  async getCurrentRound(roomId: number): Promise<Round | undefined> {
    ...
    const [latest] = await db.select().from(rounds)
  ...
  async getRound(roomId: number, roundNumber: number): Promise<Round | undefined> {
  ...
  async submitAnswers(roundId: number, playerId: number, submissions: Record<string, string>): Promise<void> {
    // Upsert answers? Or delete old ones?
    // Let's delete old ones for this round/player first to be safe (simple overwrite)
    await db.delete(answers).where(and(eq(answers.roundId, roundId), eq(answers.playerId, playerId)));
  ...
  async updateAnswerValidation(id: number, isValid: boolean, points: number, reason?: string): Promise<Answer> {
    const [answer] = await db.update(answers).set({ isValid, points, validationReason: reason }).where(eq(answers.id, id)).returning();
    return answer;
  }
}
